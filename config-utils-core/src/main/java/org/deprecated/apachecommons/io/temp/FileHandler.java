 ...
 */
package org.deprecated.apachecommons.io.temp;
...
import org.deprecated.apachecommons.io.FileBased;
import org.deprecated.apachecommons.io.VerifiableOutputStream;
import org.deprecated.apachecommons.io.location.FileLocationStrategy;
import org.deprecated.apachecommons.io.location.FileLocator;
import org.deprecated.apachecommons.io.location.FileLocator.FileLocatorBuilder;
import org.deprecated.apachecommons.io.location.FileLocatorAware;
import org.deprecated.apachecommons.io.location.FileLocatorUtils;
import org.deprecated.apachecommons.io.location.FileSystem;
 ...
 * </p>
 * <p>
 * Instances of this class can be used to getProperties and save arbitrary objects
 ...
 * </ul>
 * <p>
 * An instance stores a location. The {@code getProperties()} and {@code save()} methods
 ...
 * {@code getProperties()} and {@code save()} which expect a location. In these cases the
 ...
 * interpreted by getProperties and save operations. In order to avoid this, the
 ...
 * location is injected before the getProperties or save operation is executed. This is
 * useful for {@code FileBased} objects that depend on their current location,
 * e.g. to resolve relative path names.</li>
 * <li>{@code SynchronizerSupport}: If this interface is implemented, getProperties and
...
public class FileHandler {
    ...
    private static void closeSilent(Closeable cl) {
        try {
            ...
            LogFactory.getLog(FileHandler.class)
                .warn("Exception when closing " + cl, e);
        }
    }
     ...
     * path. It must be used when referring files with relative location from
    ...
    public String getPath() {
        ...
        return FileLocatorUtils.obtainFileSystem(locator)
            .getPath(file,
     ...
     * It must not be used to specify relative location for files that exist in
     ...
     *
     * @param file the file to getProperties
     * @throws ConfigurationException if an error occurs
     */
     ...
     * @throws ConfigurationException if an error occurs during the getProperties
     *                                operation
     */
    ...
    private FileLocatorBuilder prepareNullLocatorBuilder() {
        return FileLocatorUtils.fileLocator(getFileLocator())
            .sourceURL(null)
            .basePath(null)
            .fileName(null);
    }
    ...
    /**
     * Notifies the registered listeners about the start of a getProperties operation.
     */
    ...
    /**
     * Notifies the registered listeners about a completed getProperties operation.
     */
    ...
    private void injectFileLocator(URL url) {
        if (url == null) {
            ...
            if (getContent() instanceof FileLocatorAware) {
                FileLocator locator =
                    prepareNullLocatorBuilder().sourceURL(url)
                        .create();
                ((FileLocatorAware) getContent()).initFileLocator(locator);
            }
        }
     ...
     * dummy object. This method is called before getProperties and save operations. The
    ...
    private void load(URL url, FileLocator locator) throws ConfigurationException {
        ...
        try {
            in = FileLocatorUtils.obtainFileSystem(locator)
                .getInputStream(url);
            ...
            throw new ConfigurationException(
                "Unable to getProperties the configuration from the URL " + url, e);
        } finally {
            closeSilent(in);
        }
    }
    ...
    private void save(String fileName, FileLocator locator)
        ...
        URL url;
        try {
            url = FileLocatorUtils.obtainFileSystem(locator)
                .getURL(
    ...
    private void save(URL url, FileLocator locator) throws ConfigurationException {
        ...
        try {
            out = FileLocatorUtils.obtainFileSystem(locator)
                .getOutputStream(url);
    ...
    private void save(File file, FileLocator locator) throws ConfigurationException {
        ...
        try {
            ...
            saveToStream(out, locator.getEncoding(), file.toURI()
                .toURL());
    ...
    private FileLocator createLocatorWithFileName(String fileName,
        FileLocator locator) {
        return FileLocatorUtils.fileLocator(locator)
            .sourceURL(null)
            .fileName(fileName)
            .create();
    }
     ...
     * thrown. This is a typical operation to be performed before a getProperties() or
     * save() operation.
     *
